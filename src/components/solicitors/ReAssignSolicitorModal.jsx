...
const ReAssignSolicitorModal = ({ open, createdRequest, handleClose }) => {
  const { user, showToast } = useContext(UserContext);

  const sortBy = "solicitorApprovalStatus";
  const sortOrder = "ASC";

  const { data: ReAssignSolicitors } = useQuery({
    queryKey: ["re-assignSolicitors", sortOrder, sortBy],
    queryFn: () =>
      getAllSolicitorFn({
        page: 0,
        size: 50,
  ...
  const { mutate: reAssignAsolicitor, isLoading } = useMutation({
    mutationFn: assignRequestToSolicitorFn,
    onSuccess: () => {
      handleClose();
      showToast({
        severity: "success",
        message: "Request succesfuly re-assigned",
      });
    },
    onError: (error) => {
      showToast({
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      // console.log("FromPayload: ", payload)
      // reAssignAsolicitor({
  ...
  const sideDetails = [
    ...
    { label: "Solicitor", value: createdRequest?.companyName },
  ];
  ...
  return (
    ...
    >
      <div className="absolute w-[70%] top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] bg-white rounded-[10px] p-[3rem]">
        ...
        />
        <div className="">
          <h1 className="text-xl font-bold mb-3">Requestued Details</h1>
              ...
              <div className="grid grid-cols-2 mt-5 px-5">
                {sideDetails?.map(({ label, value }) => (
                  <div key={label}>
                    ...
                    <p
                      className={`text-[20px] fone-medium ${
                        value === "unassigned"
...
export default ReAssignSolicitorModal;
